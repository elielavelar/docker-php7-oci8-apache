...
class Syslog extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasMany(Syslogdetail::class, ['IdSysLog' => 'Id']);
    }
    ...
    public function afterFind() {
        $this->transactionModelName = $this->IdTransactionModel ? $this->transactionModel->registredModel->KeyWord: $this->transactionModelName;
    ...
    public function setDetail($detail = []){
        try {
            foreach ($detail as $key => $values ){
                ...
                $det->Attribute = $key;
                $det->Value = !empty($values['VALUE']) ? strval($values['VALUE']) : null;
                $det->OldValue = !empty($values['OLDVALUE']) ? strval($values['OLDVALUE']) : null;
                if($det->save()){
