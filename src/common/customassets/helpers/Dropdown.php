...
class Dropdown extends Widget
{
    ...
    {
        $tag = ArrayHelper::getValue($options, 'tag', $this->tag);
        $tagChildren = ArrayHelper::getValue($options, 'tagChildren', $this->tagChildren);
        $lines = [];
        foreach ($items as $item) {
            ...
            if (empty($item['items'])) {
                if ($url === null) {
                    $content = Html::tag($tagChildren, $label, ['class' =>  'dropdown-header '.ArrayHelper::getValue($itemOptions, 'class', '')]);
                } else {
                ...
                $lines[] = Html::beginTag($tag, array_merge_recursive(['class' => ['dropdown'], 'aria-expanded' => 'false'], $itemOptions));
